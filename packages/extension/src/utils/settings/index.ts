...
export type SettingsGetFunction = {
  /** すべての設定を取得 */
  (): Promise<SettingsItems>

  /** 1つの設定を取得 */
  <Key extends SettingsKey>(key: Key): Promise<SettingsItems[Key]>

  /** 複数の設定を取得 */
  <Keys extends SettingsKey[]>(
    ...keys: Keys
  ): Promise<{
    [key in Keys[number]]: SettingsItems[key]
  }>
}
...
export type SettingsSetFunction = <Key extends SettingsKey>(
  key: Key,
  value: SettingsItems[Key] | null | undefined
) => Promise<void>
...
export type SettingsWatch = <Key extends SettingsKey>(
  key: Key,
  callback: (value: SettingsItems[Key]) => void
) => () => void
